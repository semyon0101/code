...
class OverFlow:
    ...
    def update(self):
        if self.all_width > self.rect.y + self.rect.size[1]:
            ...
            if pg.mouse.get_pressed(num_buttons=3)[0] and rect.collidepoint(pg.mouse.get_pos()):
                self.active = True
...
class Text:
    ...
    def update_1(self):
        ...
        for i in range(len(self.good_text)):
            self.wights_line.append(self.FONT.render(self.good_text[i], True, (0, 0, 0)).get_width())
    ...
    def handle_event(self, _event):
        if _event.type == pg.MOUSEBUTTONDOWN:
            ...
            if (keys[pg.K_RCTRL] or keys[pg.K_LCTRL]) and keys[pg.K_c] and self.copy:
                text = ""
                last_pos = (0, 0)
                for pos in list(self.outline.keys()):
                    if not last_pos[1] == pos[1]:
                        last_pos = pos
                        text += "\n"
                        continue
                    if not list(self.outline.keys())[0] == pos:
                        text += self.good_text[pos[1]][pos[0] - 1]
    ...
    def update(self):
        if pg.mouse.get_pressed(num_buttons=3)[0] and self.copy:
            if self.old_click[0]:
                ...
                for j in range(len(self.good_text)):
                    for i in range(len(self.good_text[j])):

                        first_rect = pg.Rect((0, 0,
                                              pg.mouse.get_pos()[0] - self.old_click[1][0],
                                              pg.mouse.get_pos()[1] - self.old_click[1][1]))
                        second_rect = pg.Rect((self.wights_mark[j][i] + 5 + self.pos[0] - self.old_click[1][0],
                                               5 + self.pos[1] - self.old_click[1][
                                                   1] + text.get_height() * j + self.border,
                                               self.wights_mark[j][i] + 5 + self.pos[0] - self.old_click[1][0] +
                                               self.wights_mark[j][i + 1] - self.wights_mark[j][i],
                                               text.get_height() * (j + 1) + 5 + self.pos[1] - self.old_click[1][
                                                   1] + self.border))

                        if ((first_rect.colliderect(second_rect) or second_rect.colliderect(first_rect))):
                            self.outline[(i, j)] = True
                            self.outline[(i + 1, j)] = True
                        # elif (not abs(first_rect.size[0]) == first_rect.size[0] and
                        #         abs(first_rect.size[1]) == first_rect.size[1] and i == 0 and
                        #         second_rect.y):
                        #     pass
